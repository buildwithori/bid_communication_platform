...
> = {
  global: {
    ...
    tone: "green",
    description:
      "Inherited because the tool is available to all entrepreneurs.",
  },
  programme: {
    label: "Programme",
    tone: "blue",
...
export function ManageEntrepreneurToolsModal({
  ...
  const currentEntrepreneur =
    entrepreneurs.find((item) => item.id === entrepreneur.id) ?? entrepreneur;
  const [query, setQuery] = React.useState("");
  const debouncedQuery = useDebouncedValue(query.trim());
  const [typeFilter, setTypeFilter] = React.useState<"all" | ToolType>("all");
  const [sourceFilter, setSourceFilter] = React.useState<
    "all" | EntrepreneurToolAccessSource
  >("all");
  ...
  const publishedTools = React.useMemo(
    () => tools.filter((tool) => getToolStatus(tool) === "published"),
    [],
  );
  ...
  const visibleTools = React.useMemo(
    () =>
      publishedTools.filter(
        (tool) =>
          getEntrepreneurToolAccessSource(tool, currentEntrepreneur) !== "none",
      ),
  ...
  const addOptions = React.useMemo(
    () =>
      publishedTools
        .filter(
          (tool) =>
            getEntrepreneurToolAccessSource(tool, currentEntrepreneur) ===
            "none",
        )
  ...
  const filteredTools = React.useMemo(() => {
    ...
    return visibleTools.filter((tool) => {
      ...
      const matchesQuery =
        !needle ||
        [tool.name, tool.description, sourceMeta[source].label, audience.detail]
          .join(" ")
          .toLowerCase()
          .includes(needle);
  ...
  const patchToolAccess = (
    next: { addedToolIds?: string[]; blockedToolIds?: string[] },
    message: string,
  ) => {
  ...
  const columns: Column<Tool>[] = [
    {
      ...
      cell: (tool) => (
        <RowActions
          actions={rowActions(tool, currentEntrepreneur, removeAccess)}
        />
      ),
      className: "w-[84px]",
    },
  ...
  return (
    <Modal
      ...
      title="Manage entrepreneur tools"
      width="xl"
    >
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
            <div>
              ...
              <div className="mt-1 text-sm text-ink-muted">
                {currentEntrepreneur.representative} ·{" "}
                {currentEntrepreneur.email}
              </div>
            </div>
          ...
          <div className="mb-3 flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <div className="font-semibold text-ink">
                Add individual tool access
              </div>
              <div className="text-sm text-ink-muted">
                Use this only for exceptions. Global and programme access are
          ...
          <TableToolbar>
            <div>
              ...
              <div className="mt-0.5 text-sm text-ink-muted">
                Search, filter, or remove tools from this entrepreneur’s
                workspace.
              </div>
            </div>
          ...
          <div className="rounded-2xl border border-line bg-white p-4">
            <div className="mb-3 flex items-start justify-between gap-3">
              <div>
                ...
                <div className="text-sm text-ink-muted">
                  These are inherited tools that have been removed for this
                  entrepreneur.
                </div>
              </div>
            ...
            <div className="grid gap-2">
              {blockedTools.map((tool) => (
                <div
                  key={tool.id}
                  className="flex flex-col gap-3 rounded-xl border border-line bg-surface-subtle px-3 py-3 sm:flex-row sm:items-center sm:justify-between"
                >
...
function ToolCell({
  ...
  return (
    <div
      className={cn(
        "flex min-w-[260px] items-start gap-3",
        compact && "min-w-0",
      )}
    >
      <span
        className={cn(
          "mt-0.5 grid shrink-0 place-items-center rounded-xl",
