...
const contentTypeMeta: Record<
  ContentItem['type'],
  { label: string; icon: React.ElementType; tone: 'brand' | 'blue' | 'green'; bg: string; fg: string }
> = {
...
function ModuleDetailModalContent({
  ...
  const usedInPrograms = React.useMemo(
    () => (module ? programs.filter((item) => item.moduleIds.includes(module.id)) : []),
    [module, programs],
  );
  ...
  const contentColumns = React.useMemo<Column<ContentItem>[]>(
    () => [
      ...
      },
      {
        key: 'type',
        header: 'Type',
        cell: (item) => <Badge tone={contentTypeMeta[item.type].tone}>{contentTypeMeta[item.type].label}</Badge>,
        className: 'min-w-[120px]',
      },
      {
        key: 'trainer',
        header: 'Trainer owner',
        cell: (item) => trainers.find((trainer) => trainer.id === item.trainerId)?.fullName ?? 'Not assigned',
      ...
      },
      {
        ...
        cell: (item) => {
          ...
          return <span className="block max-w-[260px] truncate text-sm text-ink-muted">{sourceLabel}</span>;
        },
  ...
  const usageColumns = React.useMemo<Column<Program>[]>(
    () => [
        ...
        cell: (item) => (
          <div className="min-w-0">
            ...
            <div className="mt-1 text-sm text-ink-muted">
              {item.id === program?.id ? 'Current programme' : item.accessType === 'free' ? 'Free programme' : 'Assigned programme'}
            </div>
          </div>
        ),
        className: 'min-w-[320px]',
      },
      {
        key: 'status',
        header: 'Status',
        cell: (item) => <Badge tone="neutral">{getProgrammeStatusLabel(getProgrammeStatus(item))}</Badge>,
        className: 'min-w-[140px]',
      },
      {
        key: 'access',
        header: 'Access',
        cell: (item) => <Badge tone={item.accessType === 'free' ? 'blue' : 'brand'}>{item.accessType === 'free' ? 'Free' : 'Assigned'}</Badge>,
        className: 'min-w-[130px]',
      },
      {
        key: 'timeline',
        header: 'Timeline',
        cell: (item) => `${formatProgrammeDate(item.startDate)} - ${formatProgrammeDate(item.endDate)}`,
        className: 'min-w-[220px]',
      },
    ],
    [program?.id],
  ...
  return (
    <Modal open={open} onOpenChange={onOpenChange} title="Module details" width="xl">
      <div className="space-y-4">
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                ...
                {module.reuseCount ? <Badge tone="blue">Reused in {usedInPrograms.length} programmes</Badge> : null}
              </div>
              <h3 className="mt-3 text-xl font-semibold tracking-[-0.01em] text-ink">{module.title}</h3>
              {module.description ? (
                <p className="mt-2 max-w-3xl text-sm leading-6 text-ink-muted">{module.description}</p>
              ) : (
                <p className="mt-2 text-sm text-ink-muted">No module description has been added yet.</p>
              )}
            </div>
            {!readOnly && (
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
          <ModuleMetric label="Programme" value={program?.name ?? 'Not linked'} />
          <ModuleMetric label="Attached content" value={items.length} helper={`${contentCounts.video} videos, ${contentCounts.pdf} PDFs, ${contentCounts.tool} tools`} />
          <ModuleMetric label="Readiness" value={ready ? 'Ready' : 'Needs content'} progress={ready ? 100 : 0} />
          <ModuleMetric label="Used in" value={`${usedInPrograms.length} programme${usedInPrograms.length === 1 ? '' : 's'}`} />
        </div>
        ...
        <div className="rounded-xl border border-line bg-white p-4">
          ...
          {items.length > 0 ? (
            ...
            <div className="mt-4 rounded-xl border border-dashed border-line bg-surface-subtle px-4 py-8 text-center">
              ...
              <div className="mt-3 font-medium text-ink">No content has been attached</div>
              <p className="mx-auto mt-1 max-w-md text-sm leading-6 text-ink-muted">
                Add at least one video, PDF, or online tool before this module is useful to entrepreneurs.
              </p>
              {!readOnly && (
                <Button
                  className="mt-4"
        ...
        <div className="rounded-xl border border-line bg-white p-4">
          <div className="flex flex-col gap-1">
            ...
            <div className="text-sm text-ink-muted">
              Programmes using this module. Review this before changing shared content.
            </div>
          </div>
          <div className="mt-4">
            <TableToolbar>
              <div>
                ...
                <div className="mt-0.5 text-sm text-ink-muted">
                  {filteredUsage.length} of {usedInPrograms.length} programmes shown
                </div>
              </div>
              <div className="w-full sm:w-[360px]">
                <TableFilterInput
