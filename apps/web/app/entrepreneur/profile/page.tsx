...
export default function EntrepreneurProfilePage() {
  ...
  return (
    <>
      <section className="mb-4 overflow-hidden rounded-2xl border border-bid/15 bg-gradient-to-r from-bid-mid via-bid to-bid-mid p-5 text-white shadow-sm dark:border-white/15 dark:from-bid-light dark:via-bid-dark dark:to-bid-light">
        <div className="flex flex-col gap-5 sm:flex-row sm:items-center">
          <Avatar
            initials={initials(record.firstName, record.lastName, record.email)}
            size={68}
            tone="brand"
            className="border-2 border-white/40 bg-white/20 text-white dark:border-white/60 dark:bg-black/15"
          />
          <div className="min-w-0">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {record.sector ? (
                <span className="rounded-full border border-transparent bg-white/15 px-2.5 py-1 text-xs dark:border-white/20 dark:bg-black/15 dark:text-white/90">
                  {record.sector.name}
                </span>
              ) : null}
              {record.stage ? (
...
function HeaderMetric({ label, value }: { label: string; value: string }) {
  return (
    <div className="min-w-[120px] rounded-xl border border-transparent bg-white/10 px-4 py-3 text-center backdrop-blur dark:border-white/15 dark:bg-black/20 dark:shadow-sm">
