      ...
      <Card className="mt-4">
        ...
        <div className="grid gap-3 lg:grid-cols-3">
          {data.upcomingSessions.map((session) => (
            <Link key={session.id} href={`${routes.trainer.sessions}?sessionId=${encodeURIComponent(session.id)}`} className="flex min-h-[164px] flex-col rounded-xl border border-line bg-surface-subtle p-4 text-left transition hover:border-bid/30 hover:bg-card focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-bid/20">
