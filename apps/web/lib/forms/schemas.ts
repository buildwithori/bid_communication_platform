...
export const programSchema = z
  .object({
    ...
    maxEntrepreneurs: z.string().refine((value) => {
...
export const periodicUpdateSchema = z.object({
  programmeId: z
    .string()
    .min(1, "Choose whether this update is company-wide or programme-specific"),
...
export const toolRequestSchema = z.object({
  ...
  reason: z
    .string()
    .trim()
    .min(10, "Explain the business need in at least 10 characters"),
});
...
export const programmeGoalSchema = z
  .object({
    programmeId: z.string().optional(),
    goalType: z.string().min(1, "Select a goal type"),
  ...
  .superRefine((values, ctx) => {
    if (
      ["programme-completion", "milestone"].includes(values.goalType) &&
      !values.programmeId
    ) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
...
export type EntrepreneurOnboardingForm = z.infer<
  typeof entrepreneurOnboardingSchema
>;
...
export type AcceptAdminInvitationForm = z.infer<
...
export const contentItemSchema = z
  ...
  .refine(
    (values) =>
      values.type !== "video" || Boolean(values.videoFileName?.trim()),
    {
  ...
  .refine(
    (values) =>
      !["pdf", "excel"].includes(values.type) ||
      Boolean(values.pdfFileName?.trim()),
    {
      message: "Attach a PDF or Excel learning file",
      path: ["pdfFileName"],
    },
  )
  .refine(
    (values) =>
      ...
      Boolean(values.linkedToolId?.trim()),
    {
  ...
  .refine(
    (values) =>
      values.type !== "tool" ||
      values.toolSource !== "custom" ||
      z.string().url().safeParse(values.toolUrl).success,
    {
...
export const requiredDeliverableSchema = z
  .object({
    ...
    dueType: z
      .enum(["fixed-date", "module-completion", "recurring"])
      .default("fixed-date"),
  ...
  .refine(
    (values) =>
      values.dueType !== "fixed-date" || Boolean(values.dueDate?.trim()),
    {
  ...
  .refine(
    (values) =>
      values.dueType !== "recurring" ||
      Boolean(values.recurringCadence?.trim()),
    {
