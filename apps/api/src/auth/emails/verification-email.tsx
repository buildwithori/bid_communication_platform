...
export type VerificationEmailProps = { name: string; url: string; logoUrl: string };

export function VerificationEmail({ name = 'Amara', url = 'http://localhost:3000/auth/verify-email?token=preview-token', logoUrl = 'http://localhost:3000/bid-logo.png' }: VerificationEmailProps) {
  return <BidActionEmail preview="Verify your BID Hub email" heading="Verify your email" greeting={`Hello ${name},`} body="Confirm your email address to activate your entrepreneur workspace." actionLabel="Verify email" actionUrl={url} logoUrl={logoUrl} expiryNote="This link expires in 24 hours. If you did not create this account, you can safely ignore this email." />;
}
