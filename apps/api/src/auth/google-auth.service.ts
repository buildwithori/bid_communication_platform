...
export class GoogleAuthService {
  ...
  async handleCallback(
    ...
    const { tokens } = await this.integration.trackOutbound(
      {
        ...
        operation: "authentication.token_exchange",
        method: "POST",
      },
    ...
    const ticket = await this.integration.trackOutbound(
      {
        provider: "google_oauth",
        operation: "authentication.identity_verify",
        method: "POST",
      },
      () =>
    ...
    let user = await this.prisma.oAuthAccount
      .findUnique({
        where: {
    ...
    if (!user) {
      ...
      if (!byEmail && mode === "login") {
        throw new UnauthorizedException(
          "No BID Hub account exists for this Google email.",
        );
      }
      if (byEmail && byEmail.role !== UserRole.entrepreneur) {
        throw new ForbiddenException(
          "Google account entry is available to entrepreneur accounts only.",
        );
      }
    ...
    return {
      user: this.serializeUser(user, onboardingRequired),
      sessionToken,
      onboardingRequired,
    };
  }
  ...
  async getOnboarding(userId: string) {
    ...
    if (user.role !== UserRole.entrepreneur)
      throw new ForbiddenException(
        "Onboarding is only available to entrepreneur accounts.",
      );
    return {
      user: this.serializeUser(user, await this.onboardingRequired(userId)),
    };
  }
  ...
  async completeOnboarding(userId: string, dto: GoogleOnboardingDto) {
    ...
    if (
      ...
      throw new ForbiddenException(
        "Onboarding details do not match the authenticated entrepreneur.",
      );
    }
    ...
    if (completedNow) {
      await this.email.sendWelcome(
        user.email,
        firstName?.trim() || user.email.split("@")[0],
      );
    }
  ...
  private redirectUri() {
    return (
      ...
      `${this.config.getOrThrow<string>("API_PUBLIC_URL").replace(/\/$/, "")}/api/auth/google/callback`
    );
  }
