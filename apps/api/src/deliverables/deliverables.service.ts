...
import { DeliverableInstanceStatus, DeliverableReviewDecision, FileAssetUsage, Prisma, User, UserRole } from '@prisma/client';
...
const reviewInclude = { reviewer: { select: { ...userSelect, role: true } } } satisfies Prisma.DeliverableReviewInclude;
const submissionInclude = {
  submittedBy: { select: userSelect },
  fileAsset: { select: { id: true, originalFilename: true, mimeType: true, sizeBytes: true, status: true } },
  reviews: { orderBy: [{ createdAt: 'desc' as const }, { id: 'desc' as const }], take: 1, include: reviewInclude },
...
type DeliverableInstanceRow = Prisma.DeliverableInstanceGetPayload<{ include: typeof deliverableInstanceInclude }>;
type SubmissionRow = Prisma.DeliverableSubmissionGetPayload<{ include: typeof submissionInclude }>;
type FeedbackRow = Prisma.DeliverableReviewGetPayload<{ include: typeof feedbackInclude }>;
...
export class DeliverablesService {
  ...
  async listInstances(user: User, query: DeliverableInstanceQueryDto) {
    ...
    const unreadRows = pageIds.length
      ? await this.prisma.deliverableInstance.findMany({
          where: { id: { in: pageIds }, submissions: { some: { reviews: { some: { readAt: null } } } } },
          select: { id: true },
        })
      : [];
    ...
    return {
      ...
      items: rows.slice(0, take).map((row) => this.mapInstance(row, unreadIds.has(row.id))),
      totalItems,
  ...
  async listSubmissions(user: User, instanceId: string, query: DeliverableHistoryQueryDto) {
  ...
  async listFeedback(user: User, instanceId: string, query: DeliverableHistoryQueryDto) {
    await this.ensureCanReadInstance(user, instanceId);
    const take = pageSize(query);
  ...
  async submitDeliverable(user: User, instanceId: string, dto: SubmitDeliverableDto) {
    ...
    if (instance.status === DeliverableInstanceStatus.submitted) {
      throw new BadRequestException('This deliverable is already waiting for review.');
    }

    const file = await this.attachUploadedFile(user, dto.fileAssetId);
    const submitted = await this.prisma.$transaction(async (tx) => {
      await tx.deliverableSubmission.create({
        data: { instanceId, submittedById: user.id, fileAssetId: file.id, note: dto.note?.trim() || null },
      });
  ...
  async reviewSubmission(user: User, submissionId: string, dto: ReviewDeliverableDto) {
    ...
    const submission = await this.prisma.deliverableSubmission.findUnique({
      ...
      select: { id: true, instanceId: true, reviews: { select: { id: true }, take: 1 } },
    });
    ...
    if (latest?.id !== submission.id) throw new BadRequestException('Only the latest submission can be reviewed.');
    if (submission.reviews.length > 0) throw new BadRequestException('This submission has already been reviewed.');
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      await tx.deliverableInstance.update({
        ...
        data: { status: dto.decision === DeliverableReviewDecision.approved
  ...
  async markReviewRead(user: User, reviewId: string) {
    ...
    const result = await this.prisma.deliverableReview.updateMany({
      where: { id: reviewId, submission: { instance: { entrepreneurUserId: user.id } } },
      data: { readAt },
    });
    if (result.count === 0) throw new NotFoundException('Feedback was not found for this entrepreneur.');
  ...
  async updateDueDate(user: User, instanceId: string, dto: UpdateDeliverableDueDateDto) {
    ...
    const updated = await this.audit.capture(
      ...
      (tx) => tx.deliverableInstance.update({
        where: { id: instanceId },
        data: {
          dueDate,
          ...
          ...(existing.status === DeliverableInstanceStatus.not_submitted || existing.status === DeliverableInstanceStatus.overdue
  ...
  private async attachUploadedFile(user: User, fileAssetId: string) {
    const existingSubmission = await this.prisma.deliverableSubmission.findFirst({ where: { fileAssetId }, select: { id: true } });
    if (existingSubmission) throw new BadRequestException('This uploaded file has already been submitted.');
  ...
  private async ensureCanReadInstance(user: User, instanceId: string) {
    const count = await this.prisma.deliverableInstance.count({ where: { id: instanceId, ...this.readScopeWhere(user) } });
    if (count === 0) throw new NotFoundException('Deliverable was not found in your access scope.');
  }

  private async ensureCanActOnInstance(user: User, instanceId: string) {
    const count = await this.prisma.deliverableInstance.count({ where: { id: instanceId, ...this.reviewScopeWhere(user) } });
    if (count === 0) throw new NotFoundException('Deliverable was not found in your review scope.');
  }
  ...
  private statusSummary(groups: Array<{ status: DeliverableInstanceStatus; _count: { _all: number } }>) {
    const counts = Object.fromEntries(Object.values(DeliverableInstanceStatus).map((status) => [status, 0])) as Record<DeliverableInstanceStatus, number>;
  ...
  private mapReviewQueueItem(instance: DeliverableInstanceRow) {
    ...
    return {
      ...
      waitingDays: latestSubmission ? this.daysSince(latestSubmission.submittedAt) : null,
    };
  }

  private mapInstance(instance: DeliverableInstanceRow, hasUnreadFeedback: boolean) {
  ...
  private mapSubmission(submission: SubmissionRow) {
    return {
      ...
      latestReview: submission.reviews[0] ? this.mapReview(submission.reviews[0]) : null,
      file: {
  ...
  private mapReview(review: Prisma.DeliverableReviewGetPayload<{ include: typeof reviewInclude }>) {
    return {
      id: review.id,
