...
- Session booking backend rule: available time slots must be filtered by selected trainer/team member, selected date, the selected admin-managed session type's duration, and existing calendar availability. The backend owns duration resolution; clients do not submit an arbitrary duration.
...
- BID booking status and external calendar attendee response are separate state. Team acceptance confirms the BID session; Google `needs_action`, `accepted`, and `tentative` responses do not replace that status. An entrepreneur Google decline or externally removed event cancels the confirmed BID session, notifies both sides, and queues provider cleanup.
...
- User-facing copy must describe business outcomes, not infrastructure. Confirmation dialogs, helper text, and success messages must not mention vendors, object storage, queues, workers, or background cleanup; keep those details in logs and operational diagnostics.
...
- Every entrepreneur tool requires its type-specific resource when it is created or updated: PDF and Excel tools require an uploaded file, and online tools require a valid link. Draft and archived tools may omit a publish-ready audience; validate partial publishing updates against the tool's persisted audience instead of treating omitted relationship IDs as empty.
...
- A published entrepreneur tool may be represented only once per module, even through different content-library items. Paginated selectors exclude existing module tools, while create and reuse mutations lock the module and enforce the rule again.
...
- Full-page skeletons are initial-load states only. Search, filtering, pagination, and URL-backed tab changes must retain the already-rendered page shell and show a compact skeleton only inside the changing results region. Filtered collection queries retain their previous successful result during query-key transitions. Page statistics and aggregates use dedicated backend summary endpoints with stable query keys, never filtered list responses, so result refetches cannot blank or flicker metric cards.
- Do not use a role-level or pathname-guessed global loading skeleton. Every non-redirect route owns a sibling `loading.tsx`; URL-backed tabs choose the matching local skeleton, data-driven modals own a body skeleton, and the persistent sidebar only skeletonizes the account identity that is still loading.
- The App Router owns branded global not-found, route-error, and root-error states. Client render failures are reported to the backend with bounded, rate-limited metadata; never include URL query parameters, form values, stack traces, tokens, or credentials in browser error telemetry.
...
- Docker environment convention: development Compose loads the ignored `.env.local`, created from `.env.docker.example`; production operators copy `.env.production.example` to the ignored root `.env`. Do not duplicate those values in service `environment` blocks, and production Compose must never load `.env.local`.
...
- Auth forms must expose standard password-manager semantics (`username`, `current-password`, and `new-password`) and offer successful email/password credentials to the browser password manager where the Credential Management API is supported. Never persist raw credentials in application storage; the browser and user retain control over whether they are saved.
...
- Programmes and Training Library are core content surfaces and must be robust:
  ...
  - Programme playback uses one reusable, in-page course player across entrepreneur learning and admin/trainer preview. Its curriculum sidebar receives the complete ordered module/content tree without pagination; this is a deliberate bounded programme-view contract, while management lists remain cursor-paginated. Video lesson changes keep a stable 16:9 stage, prefetch the next signed playback session, mount each selected player only once, and use a short loading handoff until the next video can play.
  ...
  - A content rating belongs to the entrepreneur's programme/module/content placement. Reused content assets do not share a learner rating across programmes or modules.
...
- Learner progress is tracked only for entrepreneurs. Video playback writes a user/programme/module/content-scoped local checkpoint about every four seconds and coalesces backend checkpoints to about every twenty seconds, with immediate keepalive flushes on pause, lesson change, hidden/pagehide, unmount, and completion. Resume reconciles the local checkpoint with server `lastSyncedAt`, seeks only after media metadata is ready, and selects the most recently opened lesson even when it is completed. Position-only heartbeats persist without recomputing programme aggregates; status or quantized progress changes trigger aggregate refresh. Server events remain newest-event-wins and validate bounded position/duration values.
- Programme/module completion is relative to the current ready curriculum, not permanent. Adding or reusing learning work, video readiness changes, and content/module deletion reconcile materialized progress for every affected tracked entrepreneur, so all server-owned dashboards, filters, reports, and player responses leave 100%/completed when new unfinished content is introduced. Timestamp-gated player reads repair older stale summaries.
...
- Google Calendar response sync uses authenticated expiring Events watch channels for prompt signals and cursor-paged BullMQ reconciliation as the recovery path. Webhooks contain no trusted event body; the worker fetches the event from Google before changing BID state. Channels must be renewed, webhook jobs deduplicated, and scheduled reconciliation must remain enabled because provider notifications can be delayed or dropped.
- A Google Calendar identity can belong to only one BID Hub user at a time. Google’s verified stable account ID and normalized provider email are database uniqueness boundaries. Migration keeps the earliest owner of any pre-existing duplicate connection and disconnects later duplicates. Connecting an already-claimed calendar must preserve the existing owner and show the new user a clear conflict message.
...
- Admin permanent deletion uses exact-name confirmation in both UI and API. Programme deletion removes programme-scoped entrepreneur data, deletes modules left with no programme or deliverable-rule use, and deletes content left with no surviving module use; modules and content still used elsewhere are preserved. Removing a module that is still used by another programme preserves the shared module; deleting its final programme use deletes the module plus content used only by that module, while content reused by another module remains intact. Individual deliverable-rule deletion removes its generated requirements, submissions, reviews, notifications, and uploaded files; global content deletion removes all module links, learner progress, ratings, and queues Mux/storage cleanup. External cleanup records are committed with the database deletion and drained idempotently by BullMQ.
...
- First account activation must queue a role-specific module-owned welcome email: entrepreneur after verified signup, completed Google onboarding, or accepted entrepreneur invitation; trainer and admin after their invitation is accepted. Welcome CTAs and guidance must open the recipient's own workspace, and delivery remains in the transactional BullMQ worker.
...
- Session lifecycle notification and email schedules are rendered per recipient using the recipient's saved IANA timezone, then the company timezone and `Africa/Kigali` fallback. Keep UTC start/end as the source of truth and retain the session timezone only as booking/calendar context; never reuse one recipient's formatted body for multi-recipient fanout.
- Personal IANA timezone applies to every role. Admin and trainer profile/calendar settings expose it, invitation acceptance and the first password login capture a browser-detected initial value, and later browser changes cannot silently replace an explicit saved preference. Session availability, creation, rescheduling, list presentation, and participant notices use the authenticated viewer's saved timezone, with company default then `Africa/Kigali` fallback; clients cannot override booking interpretation with an arbitrary timezone parameter.
- Session booking calendars disable dates before today in the authenticated viewer's effective timezone. Availability rejects past date ranges and session creation/rescheduling independently rejects start times that have already passed; frontend disabling is complementary UX, not the enforcement boundary.
- Company booking hours remain company policy and are interpreted in the company timezone. The admin working-hours UI uses the shared themed time selector rather than an operating-system-native time popup, so light/dark rendering remains under the application theme.
...
- Named queues currently cover audit outbox processing, notification email delivery, recurring deliverable synchronization, transactional auth/invitation email, report export generation, Mux video reconciliation, and durable external resource cleanup for Mux, object storage, and Google Calendar. Scheduled scan jobs use BullMQ Job Schedulers; processors drain bounded batches and rely on database claims/unique constraints for idempotency.
...
- The public `/api/health` readiness response is deliberately minimal. Authenticated admins can inspect the full operational snapshot at `/admin/health`, backed by the admin-only `/api/health/details` endpoint. The page is intentionally absent from navigation and is opened only by its direct route.
- Private health distinguishes `operational`, `degraded`, and `unhealthy` without allowing noncritical diagnostics to destabilize Docker readiness. It monitors disk and memory pressure, API process memory/load, queue age/completions/schedulers, stuck or recently failed videos, notification delivery, external cleanup, report exports, audit processing, Mux webhook recency, and evidence-based backup freshness. Production backups run through the explicit Compose `operations` profile; a success marker is written only after `pg_dump` completes and becomes stale after 26 hours.
...
- Production deployment is a single-origin HTTPS stack behind Caddy: the public domain serves Next.js and proxies `/api/*` to NestJS. Production Compose applies migrations, then runs a transactionally locked `production-bootstrap-v1` task before API/worker startup. PostgreSQL records completed deployment tasks so the initial production admin and core settings are created exactly once; the local demo seed is never used in production. PostgreSQL and Redis remain internal, Redis uses AOF/no-eviction, logs are bounded, and application containers run as non-root.
...
## User-facing Copy Contract (2026-07-22)

- Product copy must describe the user-visible outcome, current state, consequence, or next action. Do not expose implementation mechanics such as background workers, queues, database transactions, backend calculations, Mux, object-storage providers, webhooks, or internal identifiers in pages, modals, toasts, emails, or API errors that may reach a user.
- Keep destructive confirmations specific enough for informed consent: name the business records and files that will be deleted or preserved, but describe asynchronous cleanup simply as deletion without explaining how the system performs it.
- Useful domain states such as processing, preparing, ready, failed, and review queue may remain when they help a user decide what to do. Provider and infrastructure detail belongs in structured logs, health diagnostics, integration callbacks, and technical documentation.
- Session cards and primary session-table titles open the shared URL-linked session details modal through the `sessionId` query parameter, preserving direct-link and reload behavior.
- Local Docker API and worker startup must run the shared, locked Prisma setup in `scripts/docker-dev-entrypoint.sh`; both regenerate the client when the schema changes and safely deploy pending migrations before starting, preventing schema drift across availability, calendar, and session lifecycle requests.
