...
- Programme lifecycle is derived from business fields, not a loose mock status. Archive wins over every other status: `archivedAt` -> Archived, no `publishedAt` -> Draft, `completedAt` or passed end date -> Completed, future start date -> Scheduled, otherwise Active.
...
- Active programmes use "End and archive" because archiving active work first marks it completed. Completed programmes can be archived or reopened. Restoring an archived programme only removes archive fields and returns it to its underlying lifecycle state.
