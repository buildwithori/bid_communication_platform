...
- Learner progress is tracked only for entrepreneurs. Video playback writes a user/programme/module/content-scoped local checkpoint about every four seconds and coalesces backend checkpoints to about every twenty seconds, with immediate keepalive flushes on pause, lesson change, hidden/pagehide, unmount, and completion. Resume reconciles the local checkpoint with server `lastSyncedAt`, seeks only after media metadata is ready, and selects the most recently opened unfinished lesson. Position-only heartbeats persist without recomputing programme aggregates; status or quantized progress changes trigger aggregate refresh. Server events remain newest-event-wins and validate bounded position/duration values.
