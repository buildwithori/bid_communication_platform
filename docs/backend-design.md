...
- A module can be reused across programmes. A content item may appear only once in any programme curriculum; content-reuse programme and module lookups must exclude destinations that already contain it, including duplicates introduced indirectly through a shared module. The attach mutation must recheck this rule transactionally across every programme using the target module. Removing a module from one of several programmes preserves the reusable module. Removing its final programme link deletes the module and any content item whose final module reference is that module; content still referenced by another module is preserved. Mux and object-storage deletion for exclusively owned content is recorded transactionally and performed by the background cleanup worker.
...
- Curriculum changes must reconcile every affected tracked learner's materialized module/programme progress. Adding a ready module or content item reopens a previously completed context until the new work is completed; deleting curriculum recomputes against what remains. Video readiness transitions are curriculum changes for this purpose. Player reads repair summaries created before this invariant only when curriculum timestamps show they are stale.
...
- Every entrepreneur tool requires its type-specific resource when it is created or updated: PDF and Excel tools require an uploaded file asset, while online tools require a valid resource link. Draft and archived tools may omit a publish-ready audience. Publishing must validate the complete effective audience, including persisted programme and entrepreneur access when a partial update changes only the status.
...
- `DELETE /programmes/:id` with exact-name confirmation
...
Permanent deletion is admin-only and separately guarded from archive/restore. Programme deletion removes programme-scoped enrolment, learning progress, goals, updates, deliverables, submissions, sessions, reports, and notifications. It also deletes modules left with no programme link or deliverable-rule dependency and content left with no surviving module reference; modules and content still used elsewhere are preserved. Removing a module from one of several programmes preserves the shared module and content; removing its final programme link deletes the module and content items not referenced by another module. An individual deliverable rule may also be permanently deleted with exact-name confirmation; its generated entrepreneur requirements, submissions, reviews, notifications, and stored submission files are removed transactionally. Global content deletion removes every module link, learner progress, and rating. External Mux assets, object-storage files, and Calendar events are written to a durable deletion outbox in the same database transaction and removed by an idempotent retrying BullMQ worker.
...
- `DELETE /content/items/:contentItemId` with exact-name confirmation
