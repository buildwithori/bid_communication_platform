...
Implementation status (2026-07-24): Complete. Entrepreneur booking uses lazy cursor-paginated trainer and session-type lookups plus backend-computed Google Calendar availability; specific requests are trainer-calendar scoped, while open-team slots require at least one available connected admin/trainer. Admins manage active/inactive session types and their required durations from the scalable Session types settings page. The backend resolves duration from the selected type and snapshots it on each booking, so configuration changes affect future slots without rewriting existing sessions. Create, accept, decline/opt-out, cancel, reschedule, complete, and note transitions use one audited lifecycle aggregate. Acceptance rechecks availability and atomically claims the request before a retrying BullMQ job provisions the Google event/Meet link. Provisioning state is recoverable, cursor-reconciled, staff-retryable, and independent of BID confirmation. Google still emails invitations to external attendee addresses, while every confirmed session also exposes a stable authenticated `.ics` file plus Google, Outlook, and Apple/other-calendar actions for non-Google users. BID confirmation and Google RSVP remain separate: pending, accepted, and tentative responses stay confirmed, while a declined invitation or externally removed event atomically cancels the session and notifies both sides. Authenticated expiring Google Events watch channels provide prompt signals; the bounded worker fetches authoritative event state, renews channels, and cursor-pages through every connected calendar and confirmed event. Rescheduling rechecks booking policy and live availability, updates or reprovisions the event, resets the attendee response, records immutable history, and rolls Calendar back on database conflicts. Company settings configure working days, hours, and slot interval. All roles consume feature-sliced hooks, backend search/counts/cursor pagination, lazy lookups, tailored skeletons, and guarded async actions. Internal notes remain hidden from entrepreneurs.
