...
- Session acceptance performs a final availability check and status-constrained audited claim before enqueueing Calendar provisioning. A retrying BullMQ worker creates the organizer event/Meet link after the BID confirmation commits, so a race cannot leave an orphan event. Pending/failed provisioning is cursor-reconciled, stale claims recover, terminal failures notify the owner, and staff can retry safely.
- Session reschedules store `session_reschedules` history and update the same Calendar event. Confirmed rows without an event are returned to pending provisioning; legacy placeholder URLs are never exposed.
...
- BID session status and Google attendee response are separate. Team acceptance confirms the BID session and creates the invitation; Google `needs_action`, `accepted`, and `tentative` remain confirmed. Google `declined` or an externally removed event atomically cancels the session, queues Calendar cleanup, audits the transition, and notifies both participants. Rescheduling resets the response to pending.
- Google Calendar response sync combines authenticated expiring Events watch channels with a cursor-paged BullMQ reconciliation scheduler. The webhook is only a change signal; the worker fetches the authoritative Google event. Channel renewal plus scheduled reconciliation is mandatory because push delivery is not a durable event log.
- Entrepreneurs do not need a Google account to receive or retain a session. Google
  sends the organizer invitation to any valid attendee email, while authenticated
  `GET /sessions/:id/calendar` returns a stable UID/revision `.ics` event for Apple,
  Outlook, Google, and other compatible calendars. BID session state is separate
  from external calendar provisioning state.
...
- Current queues are `bid-audit`, `bid-notification-delivery`, `bid-notification-automation`, `bid-recurring-deliverables`, `bid-transactional-email`, `bid-report-exports`, `bid-external-resource-cleanup`, `bid-video-reconciliation`, and `bid-calendar-sync`, under the `bid-hub` Redis prefix. Processor concurrency is intentionally bounded; periodic processors drain bounded database pages per job so they make progress without unbounded work.
