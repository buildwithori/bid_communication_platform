...
- Company-configured session policy supplies working days, workday bounds, slot step, and default duration. Working days and bounds are interpreted in the company default IANA timezone. Entrepreneurs may save a personal IANA timezone for availability display; precedence is personal preference, company default, then `Africa/Kigali`. Sessions persist the effective timezone as a calendar/display snapshot, and create/reschedule enforce the company policy.
